(function($){
	...
	function tourmaster_lightbox_tour_edit( lightbox_wrap ){
		...
		// step 0
		lightbox_wrap.on('change', 'select[name="tour-id"]', function(){
			...
			// get new input
			tourmaster_tour_booking_ajax(form.attr('data-ajax-url'), {
				success: function(data){
		...
		// step 2
		lightbox_wrap.on('change', 'input[name="package"]', function(){
			var form = $(this).closest('form');
			var sent_data = tourmaster_tour_input(form);
			sent_data['step'] = 2;
		...
		lightbox_wrap.on('click', 'input[type="submit"]', function(){
			var form = $(this).closest('form');
			var submit = true;
