...
class NitroPack {
    ...
    public function hasLocalCache($checkIfRequestIsAllowed = true) {
        ...
        return $this->pageCache->hasCache() && !$this->pageCache->hasExpired($this->config->PageCache->ExpireTime);
    }
    ...
    public function hasRemoteCache($layout, $checkIfRequestIsAllowed = true) {
        ...
        $resp = $this->api->getCache($this->url, $this->userAgent, $this->supportedCookiesFilter(self::getCookies()), false, $layout);
    ...
    public function fetchConfig() {
        ...
        if ($config) {
            ...
            if (Filesystem::createDir(dirname($file))) {
                return !!Filesystem::filePutContents($file, json_encode($config));
            }
        }
    ...
    public function isAllowedRequest($allowServiceRequests = false) {
        if ($this->isAJAXRequest() || !($this->isRequestMethod("GET") || $this->isRequestMethod("HEAD"))) {// TODO: Allow URLs which match a pattern in the AJAX URL whitelist
        ...
        if (!empty($_SERVER["HTTP_USER_AGENT"]) && (preg_match("~MSIE|Internet Explorer~i", $_SERVER["HTTP_USER_AGENT"]) || strpos($_SERVER["HTTP_USER_AGENT"], "Trident/7.0; rv:11.0") !== false)) { // Skip IE
            return false;
        }
    ...
    private function loadConfig() {
        ...
        if (Filesystem::fileExists($file) || $this->fetchConfig()) {
            ...
            if (empty($config->SDKVersion) || $config->SDKVersion !== NitroPack::VERSION) {
                if ($this->fetchConfig()) {
                    $config = json_decode(Filesystem::fileGetContents($file));
                } else {
