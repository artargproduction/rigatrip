<div>
  <div class="row">
    <div class="col-md-6 mb-3">
      <div class="card-overlay-blurrable" id="optimizations-widget">
        <div class="card card-d-item">
          <div class="card-body">
            <h5 class="card-title">Optimized Pages</h5>
        ...
        <div class="card card-d-item">
          <div class="card-body">
            ...
            <ul class="list-group list-group-flush">
              <li class="list-group-item px-0 d-flex justify-content-between align-items-center">Optimizations Quota <span><span data-optimizations>0</span> out of <span data-max-optimizations>0</span></span></li>
              <li class="list-group-item px-0 d-flex justify-content-between align-items-center">Disk Space <span><span data-disk-usage>0</span> out of <span data-max-disk-usage>0</span></span></li>
...
<script>
  ($ => {
    ...
    var getPlan = _ => {
      $.ajax({
        ...
        success: function(data) {
          ...
          $('[data-optimizations]').text(data.optimizations);
          $('[data-max-optimizations]').text(data.max_optimizations);
          $('[data-disk-usage]').text(data.disk_usage);
          $('[data-max-disk-usage]').text(data.max_disk_usage);
    ...
    $(document).on('click', "#disconnect-btn", e => {
      ...
      var confirmHtml = '<p>This will disconnect the plugin and NitroPack.io will stop working. Are you sure that you wish to continue?</p>';
      confirmHtml += '<p><a href="javascript:void(0);" onclick="rejectDisconnect()" class="btn btn-default btn-sm">No</a>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="confirmDisconnect()" class="btn btn-success btn-sm">Yes</p></a>';
      $("#settings-widget").cardOverlay("notify", {message: confirmHtml});
    });
